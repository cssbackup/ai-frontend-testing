...
export default function EditableSection({ children, id, label, defaultSelected = false }: EditableSectionProps) {
  ...
  const getSectionHtml = (preserveElementIds = false) => {
    const content = sectionRef.current?.querySelector("[data-editable-section-content]");
  ...
  const selectEditableElement = (event: React.MouseEvent<HTMLDivElement>) => {
    if (document.documentElement.hasAttribute("data-redesign-view-mode")) return;
    ...
    if (clicked.closest("[data-redesign-ui]")) return;

    const editableSelector = "img, h1, h2, h3, h4, h5, h6, p, span, a, button, li";
    ...
    const handleMessage = (event: MessageEvent<SectionMessage>) => {
      ...
      if (event.data?.type === "redesign-request-primary-element") {
        const primaryElement =
          ...
          document.querySelector<HTMLElement>("h3") ??
          document.querySelector<HTMLElement>("main p") ??
          document.querySelector<HTMLElement>("img") ??
          document.querySelector<HTMLElement>("p, span, a, button, li");
        if (primaryElement && sectionRef.current?.contains(primaryElement)) {
      ...
      if (typeof changes.text === "string") {
        const directTextNode = Array.from(target.childNodes).find((node) => node.nodeType === Node.TEXT_NODE);
        if (directTextNode) directTextNode.textContent = changes.text;
        else target.textContent = changes.text;
  ...
  return (
    <div
      ...
      className={`group/editable relative overflow-visible ${
        textToolbar
          ? "z-[200]"
          : (id === "header" || id === "topbar")
    ...
    >
      ...
      `}</style>
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-[70] border-2 border-dotted border-transparent transition group-hover/editable:border-blue-500 group-focus-within/editable:border-blue-500" />
      ...
      <div
        ...
        className="pointer-events-none absolute top-0 z-[80] w-max max-w-[calc(100%-16px)] opacity-0 transition-[left,transform,opacity] duration-300 ease-in-out group-hover/editable:pointer-events-auto group-hover/editable:opacity-100"
        onMouseDown={(event) => event.stopPropagation()}
        onPointerDown={(event) => event.stopPropagation()}
      >
        <div className="pointer-events-none flex w-max max-w-full flex-nowrap items-center justify-center gap-1 whitespace-nowrap rounded-b-2xl border border-t-0 border-dotted border-blue-500 bg-white p-1 pl-3 pr-2 text-[11px] text-slate-900 group-hover/editable:pointer-events-auto sm:gap-1 sm:text-sm">
          <button
            type="button"
      ...
      {canReorderSection && (
        <div
          ...
          className="pointer-events-none absolute bottom-0 left-1/2 z-[85] flex -translate-x-1/2 translate-y-1/2 items-center gap-1.5 rounded-full bg-slate-100 p-1 shadow-[0_6px_18px_rgba(15,23,42,0.12)] opacity-0 transition group-hover/editable:pointer-events-auto group-hover/editable:opacity-100"
          onMouseDown={(event) => event.stopPropagation()}
          onPointerDown={(event) => event.stopPropagation()}
        >
          <button
