...
import UserNotificationsBell from "./user-notifications-bell";
...
export default function Navbar({
  ...
  const handleNotificationHref = (href: string) => {
    if (href.includes("/notifications")) onNavigate("Notifications");
    else if (href.includes("/websites-lead")) onNavigate("Websites Lead");
    else if (href.includes("/billing")) onNavigate("Billing");
    else if (href.includes("/profile")) onNavigate("Profile");
    ...
    <header className="relative z-20 flex items-center gap-2 border-b border-zinc-200/80 bg-white/90 px-3 backdrop-blur-xl sm:gap-4 sm:px-5 lg:px-6">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <UserNotificationsBell
          onNavigateHref={handleNotificationHref}
