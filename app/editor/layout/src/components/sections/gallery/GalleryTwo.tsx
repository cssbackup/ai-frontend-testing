    ...
    <section className="bg-slate-950 px-6 py-20 text-white">
      <div className="mx-auto max-w-6xl">
        <div className="flex items-start justify-between gap-4">
          {data.title && <h2 className="text-3xl font-semibold sm:text-4xl" data-editor-inline-format-key="gallery-two:title"><InlineRichText value={data.title} formatKey="gallery-two:title" /></h2>}
        ...
        <div ref={scrollerRef} data-export-gallery-scroller className="mt-10 flex gap-4 overflow-x-auto scroll-smooth pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {items.map((item, index) => (
            <article key={`${item.image}-${index}`}>
              <GalleryImage item={item} />
              {item.title && (
                <h3 className="mt-4 text-lg font-semibold text-white" data-editor-inline-format-key={`gallery-two:${index}:title`}>
                  <InlineRichText value={item.title} formatKey={`gallery-two:${index}:title`} />
                </h3>
              )}
            </article>
          ))}
