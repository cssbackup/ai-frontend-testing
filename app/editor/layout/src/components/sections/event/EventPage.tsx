...
import { normalizeEventIndexLayout } from "../../../lib/eventLayouts";
...
const toEventSlide = (
  ...
  return {
    title,
    category: "category" in item ? item.category : "Event",
    ...
    href,
    eventDate:
      "eventDate" in item && typeof item.eventDate === "string"
        ? item.eventDate
        : "",
    eventTime:
      "eventTime" in item && typeof item.eventTime === "string"
        ? item.eventTime
        : "",
    eventType:
      "eventType" in item && item.eventType === "past" ? "past" : "upcoming",
  };
};
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
              service.eventType === "past"
                ? "bg-slate-200 text-slate-600"
                : "bg-emerald-100 text-emerald-700"
            }`}
          >
            {service.eventType === "past" ? "Past" : "Upcoming"}
          </span>
        </div>
        {(service.eventDate || service.eventTime) && (
          <p className="mt-2 text-xs font-semibold text-slate-500">
            {[service.eventDate, service.eventTime].filter(Boolean).join(" · ")}
          </p>
        )}
        <h3
...
export default function EventPage({ data = {} }: SectionProps) {
  const [startIndex, setStartIndex] = useState(0);
  const layout = normalizeEventIndexLayout(
    typeof data.layout === "string" ? data.layout : "EventPage-1",
  );
  const sideImage = data.sideImage;
  const sideImageTitle = data.sideImageTitle ?? "";
  const detailBase =
    typeof data.eventDetailBase === "string"
      ? data.eventDetailBase.trim()
      : "";
    ...
    const slides = fromItems.length
      ...
      : data.serviceSlides?.length
        ...
        : data.productSlides?.length
          ...
          : [];
    return slides
      .map((item) => toEventSlide(item, detailBase || undefined))
  ...
  // EventPage-2 — Feature spotlight
  if (layout === "EventPage-2") {
    return (
  ...
  // EventPage-3 — Card showcase
  if (layout === "EventPage-3") {
    return (
  ...
  // EventPage-4 — Grid catalog
  if (layout === "EventPage-4") {
    return (
      <main className="bg-slate-50 text-slate-950">
  ...
  // EventPage-1 — Split intro + carousel (default)
  return (
    <main className="bg-white text-slate-950">
