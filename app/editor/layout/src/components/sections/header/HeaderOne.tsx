...
export default function HeaderOne({
  ...
  const handlePageClick = (
    ...
    // Blog submenu may use a short label — resolve the real page by href.
    if (
      preview &&
    ...
    const publishedBasePath = getPublishedSiteBasePath();
    const isHomeClick =
      label.trim().toLowerCase() === "home" ||
      !trimmedHref ||
      trimmedHref === "#" ||
      (Boolean(publishedBasePath) &&
        trimmedHref.replace(/\/+$/, "") === publishedBasePath);
    if (
      isHomeClick &&
      window.location.pathname.startsWith("/published/") &&
      getPublishedSiteBasePath() &&
      ...
      // Blog/service-detail shells ignore currentPage — must leave the URL.
    ...
    // Published: navigate by href so CTA labels ("Book Visit") still open the target page.
