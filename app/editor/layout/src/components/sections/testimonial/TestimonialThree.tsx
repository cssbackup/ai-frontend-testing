    ...
    <section className="bg-[#f8fafc] px-6 py-20">
      <div className="mx-auto max-w-6xl text-center">
        <p
          ...
          data-editor-inline-format-key="testimonial-three:pretitle"
        >
          <InlineRichText
            value={data.pretitle ?? ""}
            formatKey="testimonial-three:pretitle"
          />
        </p>
        <h2
          className="mt-4 text-3xl font-semibold text-slate-950 sm:text-4xl"
          data-editor-inline-format-key="testimonial-three:title"
        >
          <InlineRichText
            value={data.title ?? ""}
            formatKey="testimonial-three:title"
          />
        </h2>
          ...
          {items.map((item, index) => (
            <article key={`${item.name}-${index}`} className="min-w-[min(280px,82vw)] snap-start rounded-xl bg-white p-5 text-left shadow-sm sm:p-6 md:min-w-[340px]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3
                    ...
                    data-editor-inline-format-key={`testimonial-three:${index}:name`}
                  >
                    <InlineRichText
                      value={item.name}
                      formatKey={`testimonial-three:${index}:name`}
                    />
                  </h3>
                  <p
                    className="text-sm text-slate-500"
                    data-editor-inline-format-key={`testimonial-three:${index}:role`}
                  >
                    <InlineRichText
                      value={item.role}
                      formatKey={`testimonial-three:${index}:role`}
                    />
                  </p>
                </div>
              </div>
              <p
                ...
                data-editor-inline-format-key={`testimonial-three:${index}:quote`}
              >
                <InlineRichText
                  value={item.quote}
                  formatKey={`testimonial-three:${index}:quote`}
                />
              </p>
              <div className="mt-5 flex items-center gap-1 text-amber-500">
                ...
                {item.rating && (
                  <span
                    className="ml-2 text-sm font-bold theme-accent"
                    data-editor-inline-format-key={`testimonial-three:${index}:rating`}
                  >
                    <InlineRichText
                      value={item.rating}
                      formatKey={`testimonial-three:${index}:rating`}
                    />
                  </span>
                )}
              </div>
