  ...
  return (
    <section className="bg-slate-950 px-6 py-20 text-white">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center lg:gap-10">
        <div>
          <p
            className="text-sm font-bold uppercase tracking-[0.18em] text-cyan-300"
            data-editor-inline-format-key="testimonial-two:pretitle"
          >
            <InlineRichText
              value={data.pretitle ?? ""}
              formatKey="testimonial-two:pretitle"
            />
          </p>
          {data.title && (
            <h2
              className="mt-4 text-3xl font-semibold sm:text-4xl"
              data-editor-inline-format-key="testimonial-two:title"
            >
              <InlineRichText
                value={data.title}
                formatKey="testimonial-two:title"
              />
            </h2>
          )}
          {data.desc && (
            <p
              ...
              data-editor-inline-format-key="testimonial-two:description"
            >
              <InlineRichText
                value={data.desc}
                formatKey="testimonial-two:description"
              />
            </p>
          )}
        ...
        <div ref={scrollerRef} className="flex snap-x gap-5 overflow-x-auto scroll-smooth pb-4">
          {items.map((item, itemIndex) => (
            <article key={`${item.name}-${itemIndex}`} className="min-w-[min(320px,82vw)] snap-start rounded-2xl bg-white p-5 text-slate-950 sm:p-8 md:min-w-[460px]">
              ...
              <p
                className="mt-6 text-xl font-semibold leading-8 before:content-['“'] after:content-['”'] sm:text-2xl sm:leading-10"
                data-editor-inline-format-key={`testimonial-two:${itemIndex}:quote`}
              >
                <InlineRichText
                  value={item.quote}
                  formatKey={`testimonial-two:${itemIndex}:quote`}
                />
              </p>
              <div className="mt-8 flex items-center gap-4">
                ...
                <div>
                  <h3
                    className="font-bold"
                    data-editor-inline-format-key={`testimonial-two:${itemIndex}:name`}
                  >
                    <InlineRichText
                      value={item.name}
                      formatKey={`testimonial-two:${itemIndex}:name`}
                    />
                  </h3>
                  <p
                    className="text-sm text-slate-500"
                    data-editor-inline-format-key={`testimonial-two:${itemIndex}:role`}
                  >
                    <InlineRichText
                      value={item.role}
                      formatKey={`testimonial-two:${itemIndex}:role`}
                    />
                  </p>
                </div>
              </div>
