...
import { normalizeTeamDetailLayout } from "../../../lib/teamLayouts";
...
type TeamDetailArticleProps = {
  teamMember: TeamDetailContent;
  backSlot?: ReactNode;
};
...
const HtmlOrText = ({
  ...
  if (/<\/?[a-z][\s\S]*>/i.test(trimmed)) {
    return (
      <div
        className={`team-detail-prose manager-detail-prose max-w-none text-slate-700 ${className}`}
...
export default function TeamDetailArticle({
  teamMember,
  backSlot,
}: TeamDetailArticleProps) {
  const layout = normalizeTeamDetailLayout(teamMember.layout);
  const title = teamMember.title || "Untitled team member";
  const category = teamMember.category?.trim() || "";
  const excerpt = teamMember.excerpt?.trim() || "";
  const content =
    teamMember.content?.trim() ||
    excerpt ||
    "No content has been added to this team member yet.";
  const image = teamMember.image?.trim() || "/bg1.jpg";

  if (layout === "TeamDetail-2") {
    return (
