...
import { normalizeTeamIndexLayout } from "../../../lib/teamLayouts";
...
const toTeamSlide = (
  ...
  return {
    title,
    category: "category" in item ? item.category : "Teams",
    ...
    >
      ...
      <div className={compact ? "p-4" : "p-5"}>
        <p
          ...
          data-editor-inline-format-key={`team-page:${index}:category`}
        >
          <InlineRichText
            value={item.category}
            formatKey={`team-page:${index}:category`}
          />
        </p>
        <h3
          className={`font-black ${compact ? "mt-2 text-lg" : "mt-3 text-xl"}`}
          data-editor-inline-format-key={`team-page:${index}:title`}
        >
          <InlineRichText
            value={item.title}
            formatKey={`team-page:${index}:title`}
          />
        </h3>
        <p
          className={`text-sm leading-7 text-slate-600 ${compact ? "mt-2 line-clamp-2" : "mt-3"}`}
          data-editor-inline-format-key={`team-page:${index}:description`}
        >
          <InlineRichText
            value={item.desc}
            formatKey={`team-page:${index}:description`}
          />
        </p>
        {item.href ? (
...
const IntroBlock = ({ data }: { data: NonNullable<SectionProps["data"]> }) => (
  <div>
    <p
      ...
      data-editor-inline-format-key="team-page:pretitle"
    >
      <InlineRichText
        value={data.pretitle ?? ""}
        formatKey="team-page:pretitle"
      />
    </p>
    <h1
      className="mt-4 text-4xl font-black leading-tight tracking-tight md:text-5xl"
      data-editor-inline-format-key="team-page:title"
    >
      <InlineRichText
        value={data.title ?? ""}
        formatKey="team-page:title"
      />
    </h1>
    <div className="mt-5 space-y-4 text-base leading-8 text-slate-600">
      <p data-editor-inline-format-key="team-page:description">
        <InlineRichText
          value={data.desc ?? ""}
          formatKey="team-page:description"
        />
      </p>
      {data.desc2 ? (
        <p data-editor-inline-format-key="team-page:description-secondary">
          <InlineRichText
            value={data.desc2}
            formatKey="team-page:description-secondary"
          />
        </p>
      ) : null}
    </div>
...
const SectionHeading = ({
  ...
  <div>
    <p
      className="text-sm font-bold uppercase tracking-[0.22em] theme-accent"
      data-editor-inline-format-key="team-page:subtitle"
    >
      <InlineRichText
        value={data.subtitle ?? ""}
        formatKey="team-page:subtitle"
      />
    </p>
    <h2
      className="mt-3 text-3xl font-black tracking-tight"
      data-editor-inline-format-key="team-page:section-title"
    >
      <InlineRichText
        value={data.productSectionTitle ?? ""}
        formatKey="team-page:section-title"
      />
    </h2>
  </div>
);

export default function TeamPage({ data = {} }: SectionProps) {
  const [startIndex, setStartIndex] = useState(0);
  const layout = normalizeTeamIndexLayout(
    typeof data.layout === "string" ? data.layout : "TeamPage-1",
  );
  const sideImage = data.sideImage;
  const sideImageTitle = data.sideImageTitle ?? "";
  const detailBase =
    typeof data.teamDetailBase === "string"
      ? data.teamDetailBase.trim()
      : "";
    ...
    const slides = fromItems.length
      ...
      : data.serviceSlides?.length
        ...
        : data.productSlides?.length
          ...
          : [];
    return slides
      .map((item) => toTeamSlide(item, detailBase || undefined))
        ...
        <section className="border-b border-slate-200 bg-white px-5 py-16 md:px-8 lg:py-20">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p
              ...
              data-editor-inline-format-key="team-page:description"
            >
              <InlineRichText
                value={data.desc ?? ""}
                formatKey="team-page:description"
