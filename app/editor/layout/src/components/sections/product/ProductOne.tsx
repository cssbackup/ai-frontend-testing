...
export default function ProductOne({ data = {}, blocks }: SectionProps) {
  ...
  const primaryTitleFormatKey = `block:${activeSlide.id}:title:primary`;
  const secondaryTitleFormatKey = `block:${activeSlide.id}:title:secondary`;
  const categoryFormatKey = `block:${activeSlide.id}:category`;
  const descriptionFormatKey = `block:${activeSlide.id}:description`;

  return (
    <section
      className="w-full bg-white"
      data-export-product-slider
    ...
    >
      <div className="mx-auto max-w-7xl bg-radial from-white via-blue-50 to-transparent px-5 py-12 md:px-10 md:py-20">
        ...
        >
          <div>
            <h1
              className="text-3xl font-bold leading-tight text-black sm:text-4xl md:text-5xl"
              data-editor-inline-format-key={primaryTitleFormatKey}
              data-export-product-title
            >
              <InlineRichText value={activeSlide.title ?? ""} formatKey={primaryTitleFormatKey} legacyOccurrence={0} />
            </h1>

            <p
              className="mt-3 text-sm text-black"
              data-editor-inline-format-key={categoryFormatKey}
              data-export-product-category
            >
              <InlineRichText value={activeSlide.category ?? ""} formatKey={categoryFormatKey} />
            </p>
            ...
            {slideButton && (
              <BlockRenderer
                block={slideButton}
                className="theme-btn mt-4 inline-block rounded px-6 py-2 text-xs font-bold"
              />
            )}
          </div>
          ...
          <div>
            <h2
              ...
              data-editor-inline-format-key={secondaryTitleFormatKey}
              data-export-product-title
            >
              <InlineRichText value={activeSlide.title ?? ""} formatKey={secondaryTitleFormatKey} legacyOccurrence={1} />
            </h2>

            <p
              className="mt-3 max-w-sm text-sm leading-relaxed text-black lg:max-w-xs lg:text-xs"
              data-editor-inline-format-key={descriptionFormatKey}
              data-export-product-desc
            >
              <InlineRichText value={activeSlide.desc ?? ""} formatKey={descriptionFormatKey} />
            </p>
            ...
            <div className="mt-8 rounded-sm bg-white p-5 shadow">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                {slideButton && (
                  <BlockRenderer
                    block={slideButton}
                    className="theme-btn rounded-lg px-5 py-3 text-xs font-bold"
                  />
                )}
              </div>
            </div>
