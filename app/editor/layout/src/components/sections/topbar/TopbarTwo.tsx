...
export default function TopbarTwo({ data = {}, blocks }: SectionProps) {
  ...
  const topbarBackground =
    data.topbarBackgroundType === "gradient"
      ...
      : topbarSolidColor;
  const topbarTextColor = data.topbarTextColor ?? "var(--secondary-text)";
  ...
  return (
    ...
    >
      <div className="mx-auto flex min-h-11 max-w-7xl flex-col items-start justify-center gap-2 px-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
        ...
        <div className="flex max-w-full flex-wrap items-center gap-x-3 gap-y-2 sm:justify-end sm:divide-x sm:divide-current/20">
          ...
          <div className="flex items-center">
            ...
            {!isHidden("socialLinks") && socialLinks.map((socialLink, index) => {
              const Icon = getSocialPlatformIcon(socialLink.label);
