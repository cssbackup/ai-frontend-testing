...
import {
  ...
  normalizeEventIndexLayout,
  EVENT_DETAIL_LAYOUTS,
  EVENT_INDEX_LAYOUTS,
  type EventLayoutOption,
} from "../layout/src/lib/eventLayouts";

type EventTab = "events" | "categories" | "templates" | "seo";
type LayoutPane = "index" | "detail";

const DEFAULT_EVENT_CATEGORY = "Event";
...
export type EventItem = {
  ...
  image: string;
  alt?: string;
  /** Detail page URL slug (`/event/{slug}`). Auto-filled from title when empty. */
  slug?: string;
  order?: number;
  active?: boolean;
  layout?: string;
  ...
  eventTime?: string;
  /** Upcoming vs past listing badge. */
  eventType?: "upcoming" | "past";
};

export type EventTypeValue = NonNullable<EventItem["eventType"]>;

const EVENT_TYPE_OPTIONS: Array<{ value: EventTypeValue; label: string }> = [
  { value: "upcoming", label: "Upcoming" },
  { value: "past", label: "Past Event" },
];

const normalizeEventType = (value?: string | null): EventTypeValue =>
  value === "past" ? "past" : "upcoming";

const createEventSlug = (label: string) =>
  label
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, "")
...
const uniqueEventSlug = (
  ...
  const base = preferred || "event";
  const taken = new Set(
    events
      .filter((item) => item.id !== excludeId)
      .map((item) =>
        (item.slug || createEventSlug(item.title) || "").toLowerCase(),
      )
      .filter(Boolean),
  );
...
const EVENT_PAGE_SEO_KEY = "Events";
...
const emptyEvent = (order = 1, detailLayout = DEFAULT_EVENT_DETAIL_LAYOUT): EventItem => ({
...
const emptyPageState = (): EventPageState => ({
  pretitle: "Our Events",
  title: "Events worth showing up for",
  subtitle: "Upcoming",
  desc: "Share upcoming events, workshops, and gatherings with your audience.",
  desc2: "",
  sideImage: "/bg1.jpg",
  sideImageTitle: "Events",
  productSectionTitle: "All events",
  layout: DEFAULT_EVENT_INDEX_LAYOUT,
  detailLayout: DEFAULT_EVENT_DETAIL_LAYOUT,
  events: [],
});

const LayoutPreviewMock = ({ layout }: { layout: EventLayoutOption }) => {
      ...
      return (
        <div className="absolute inset-x-7 bottom-6 grid grid-cols-[0.8fr_1.2fr] gap-5 text-white">
          <div>
            ...
            <p className="mt-2 text-xl font-bold">Event detail</p>
          </div>
...
export default function EventManager({
  ...
  const { siteSeoConfig, setSiteSeoConfig, pageLinks, setPageLinks } =
    usePreview();
  const eventsWebsiteEnabled = useMemo(() => {
    const link = pageLinks.find(isEventsPageLink);
    return Boolean(link && !link.hidden);
  }, [pageLinks]);

  const setEventsWebsiteEnabled = (enabled: boolean) => {
    if (enabled) {
      const hasLink = pageLinks.some(isEventsPageLink);
      setPageLinks(
        hasLink
          ? pageLinks.map((page) =>
              isEventsPageLink(page)
                ? {
                    ...page,
                    label: page.label || "Events",
                    href: page.href || "#page-events",
                    hidden: false,
                  }
                : page,
            )
      ...
      window.dispatchEvent(new CustomEvent("ai-builder-ensure-event-page"));
      window.dispatchEvent(
        new CustomEvent("ai-builder-events-website-visibility", {
          detail: { enabled: true },
        }),
      );
      return;
    ...
    setPageLinks(
      pageLinks.map((page) =>
        isEventsPageLink(page) ? { ...page, hidden: true } : page,
      ),
    );
    window.dispatchEvent(
      new CustomEvent("ai-builder-events-website-visibility", {
  ...
  const eventSeo = useMemo(
    () => getPageSeo(siteSeoConfig, EVENT_PAGE_SEO_KEY),
    [siteSeoConfig],
  );
  const patchEventSeo = (patch: Partial<SiteSeoSettings>) => {
    setSiteSeoConfig(patchPageSeo(siteSeoConfig, EVENT_PAGE_SEO_KEY, patch));
  };

  const [activeTab, setActiveTab] = useState<EventTab>("events");
  const [layoutPane, setLayoutPane] = useState<LayoutPane>("index");
  const [ready, setReady] = useState(false);
  const [pageState, setPageState] = useState<EventPageState>(emptyPageState);
  ...
  const [draft, setDraft] = useState<EventItem>(emptyEvent());
  ...
  const { aiFieldBusy, generateText, generateImage, requireCorePlanForAi } =
    useManagerAiFields({
      ...
      imageHintParts: () => [draft.title, draft.category, "event workshop"],
  ...
  const categoryStorageKey = `ai-builder-event-categories:${siteId}`;

  useEffect(() => {
    if (activeTab !== "seo") return;
    setSeoDraft({
      metaTitle: eventSeo.metaTitle || "",
      metaDescription: eventSeo.metaDescription || "",
      metaKeywords: eventSeo.metaKeywords || "",
    });
  ...
  const eventCategoriesKey = useMemo(
    () =>
      Array.from(
        new Set(
          pageState.events
            .map((item) =>
              (item.category || DEFAULT_EVENT_CATEGORY).trim(),
            )
            .filter(Boolean),
        ),
      )
  ...
  useEffect(() => {
    const handleState = (event: Event) => {
      ...
      if (!detail) return;
      const layout = normalizeEventIndexLayout(detail.layout);
      const detailLayout = normalizeEventDetailLayout(detail.detailLayout);
      setPageState({
        ...emptyPageState(),
        ...detail,
        layout,
    ...
    window.addEventListener("ai-builder-event-page-state", handleState);
    window.dispatchEvent(new CustomEvent("ai-builder-ensure-event-page"));
    // Query after ensure can patch sectionsRef, so the list is not wiped by a stale snapshot.
    window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent("ai-builder-event-page-query"));
    }, 0);

    return () => {
      window.removeEventListener("ai-builder-event-page-state", handleState);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const fromEvents = eventCategoriesKey
      ? eventCategoriesKey.split("\0")
      : [];
    const next = Array.from(
      new Set([DEFAULT_EVENT_CATEGORY, ...saved, ...fromEvents]),
    );
    setCategories(next);
  ...
  // When events gain categories, merge them in without dropping custom ones.
  useEffect(() => {
    ...
    setCategories((current) => {
      const next = Array.from(new Set([...current, ...fromEvents]));
      const unchanged =
        next.length === current.length &&
        next.every((category) => current.includes(category));
      return unchanged ? current : next;
    });
  }, [eventCategoriesKey, categoriesReady]);
  ...
  const persist = (next: EventPageState) => {
    const withLayout = {
      ...next,
      layout: normalizeEventIndexLayout(next.layout || localIndexLayout),
      detailLayout: normalizeEventDetailLayout(
        next.detailLayout || localDetailLayout,
      ),
    };
    setPageState(withLayout);
    window.dispatchEvent(
      new CustomEvent("ai-builder-event-page-update", { detail: withLayout }),
    );
  };

  const applyIndexLayout = (layoutId: string) => {
    const layout = normalizeEventIndexLayout(layoutId);
    setLocalIndexLayout(layout);
    persist({ ...pageState, layout });
  };

  const applyDetailLayout = (layoutId: string) => {
    const detailLayout = normalizeEventDetailLayout(layoutId);
    setLocalDetailLayout(detailLayout);
    persist({
      ...pageState,
      detailLayout,
      events: pageState.events.map((item) => ({
        ...item,
        layout: detailLayout,
      })),
    });
  ...
  const nextDefaultOrder = useMemo(() => {
    const maxOrder = pageState.events.reduce(
      (max, eventItem) => Math.max(max, eventItem.order ?? 0),
      0,
    );
    return maxOrder + 1;
  }, [pageState.events]);
  ...
  const filteredEvents = useMemo(() => {
    ...
    const sorted = [...pageState.events].sort(
      (a, b) => (a.order ?? 0) - (b.order ?? 0),
    );
    if (!query) return sorted;
    return sorted.filter((eventItem) =>
      [eventItem.title, eventItem.category, eventItem.desc].some((value) =>
        value.toLowerCase().includes(query),
      ),
    );
  }, [pageState.events, search]);
  ...
  const openCreate = () => {
    setEditingId(null);
    const created = emptyEvent(nextDefaultOrder, localDetailLayout);
    created.category = categories[0] || DEFAULT_EVENT_CATEGORY;
    setDraft(created);
  ...
  const openEdit = (eventItem: EventItem) => {
    ...
    setDraft({
      ...emptyEvent(1, localDetailLayout),
      ...eventItem,
      ...
      eventType: normalizeEventType(eventItem.eventType),
    });
    setShowComposer(true);
  };
  ...
  const saveEditCategory = () => {
    ...
    if (nextName !== editingCategory) {
      const renamedEvents = pageState.events.map((eventItem) =>
        (eventItem.category || DEFAULT_EVENT_CATEGORY) === editingCategory
          ? { ...eventItem, category: nextName }
          : eventItem,
      );
      persist({ ...pageState, events: renamedEvents });
      if (
        (draft.category || DEFAULT_EVENT_CATEGORY) === editingCategory
      ) {
  ...
  const saveEvent = () => {
    ...
    const slug = uniqueEventSlug(
      createEventSlug(draft.slug || title) || `event-${Date.now()}`,
      pageState.events,
      editingId,
    );
    ...
    const nextItem: EventItem = {
      ...draft,
      title,
      category: draft.category.trim() || DEFAULT_EVENT_CATEGORY,
    ...
    const events = editingId
      ? pageState.events.map((item) =>
          item.id === editingId ? nextItem : item,
        )
      : [...pageState.events, nextItem];

    const withLayout = {
      ...pageState,
      events,
      layout: normalizeEventIndexLayout(pageState.layout || localIndexLayout),
      detailLayout: normalizeEventDetailLayout(
  ...
  const deleteEvent = (id: string) => {
    const withLayout = {
      ...pageState,
      events: pageState.events.filter((item) => item.id !== id),
      layout: normalizeEventIndexLayout(pageState.layout || localIndexLayout),
      detailLayout: normalizeEventDetailLayout(
        pageState.detailLayout || localDetailLayout,
      ),
  ...
  const deleteEventsByIds = (ids: string[]) => {
    if (!ids.length) return;
    const idSet = new Set(ids);
    const withLayout = {
      ...pageState,
      events: pageState.events.filter((item) => !idSet.has(item.id)),
      layout: normalizeEventIndexLayout(pageState.layout || localIndexLayout),
      detailLayout: normalizeEventDetailLayout(
        pageState.detailLayout || localDetailLayout,
      ),
  ...
  const confirmDeleteEvent = () => {
    ...
    setSelected((current) =>
      current.filter((id) => id !== eventToDelete.id),
    );
    ...
    >
      <header className="flex h-[66px] shrink-0 items-center justify-between gap-4 border-b border-slate-200 px-5 sm:px-7">
        <div>
          ...
          <h1 id="event-manager-title" className="sr-only">
            Event page manager
          </h1>
        </div>
        <div className="flex min-w-0 flex-1 items-center justify-end gap-3 sm:gap-4">
          ...
          >
            ...
            <input
              type="checkbox"
              className="sr-only"
              checked={eventsWebsiteEnabled}
              onChange={(event) =>
                setEventsWebsiteEnabled(event.target.checked)
              }
            />
            <span
              aria-hidden="true"
              className={`relative h-6 w-11 shrink-0 rounded-full transition ${
                eventsWebsiteEnabled ? "bg-emerald-500" : "bg-slate-300"
              }`}
            >
              <span
                className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition ${
                  eventsWebsiteEnabled ? "translate-x-5" : "translate-x-0"
                }`}
              />
            </span>
          </label>
          ...
          <button
            ...
            onClick={onClose}
            className="grid h-9 w-9 place-items-center rounded-full text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
            aria-label="Close event manager"
          >
            <X size={19} />
          </button>
        </div>
      ...
      {!eventsWebsiteEnabled ? (
        <div className="shrink-0 border-b border-amber-200 bg-amber-50 px-5 py-2.5 text-sm text-amber-900 sm:px-7">
          Events are hidden on the live website. Items stay saved here —{" "}
          <span className="font-semibold">/events</span> and detail links will
        ...
        <div className="min-h-0 min-w-0 flex-1 overflow-y-auto">
              ...
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-base text-slate-600">
                    Create and manage events shown on your Event page.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    title="AI Assist — generate events"
              ...
              <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200">
                <div className="grid grid-cols-[42px_minmax(180px,1.3fr)_90px_110px_70px_90px_100px] border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">
                  <input
                    ...
                    checked={
                      ...
                      pagedEvents.every((item) =>
                        selected.includes(item.id),
                      )
                    }
                    onChange={() => {
                      const ids = pagedEvents.map((item) => item.id);
                ...
                {pagedEvents.map((item) => {
                  ...
                  const typeLabel =
                    normalizeEventType(item.eventType) === "past"
                      ? "Past"
                      : "Upcoming";
                  const whenLabel = [item.eventDate, item.eventTime]
                    .filter(Boolean)
                    .join(" · ");
                  return (
                    <div
                ...
                {filteredEvents.length === 0 && (
                  <div className="px-5 py-12 text-center text-sm text-slate-500">
                    No events yet. Add your first event to show on the page.
                  </div>
                )}

                {filteredEvents.length > LIST_PAGE_SIZE ? (
          ...
          {ready && activeTab === "categories" && (
            <section className="px-5 py-7 sm:px-8 lg:px-10">
              ...
              <p className="mt-2 text-base text-slate-600">
                Organize events into reusable categories.
              </p>
              <form
              ...
              <div className="mt-6 max-w-xl divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                {categories.map((category) => {
                  const used = pageState.events.some(
                    (item) =>
                      (item.category || DEFAULT_EVENT_CATEGORY) ===
                      category,
                  );
          ...
          {ready && activeTab === "templates" && (
            <section className="px-5 py-7 sm:px-8 lg:px-10">
              <div>
                ...
                <p className="mt-2 text-base text-slate-600">
                  Choose a listing layout for the Events page and a detail
              ...
              <p className="mt-5 text-sm font-semibold text-slate-500">
                {layoutPane === "index"
                  ? "Applied to the Events listing page for every visitor."
                  : "Applied as the default for all event detail pages."}
              </p>
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[16/9] overflow-hidden bg-slate-100">
                        <Image
                          src={layout.image}
                          alt={`${layout.name} event layout preview`}
                          fill
          ...
          {ready && activeTab === "seo" && (
            <section className="px-5 py-7 sm:px-8 lg:px-10">
              ...
              <p className="mt-2 max-w-2xl text-base text-slate-600">
                Meta tags for the Events page. Each event can also have its
              ...
              <div className="mt-7 max-w-xl space-y-4">
                <label className="block">
                  ...
                  <input
                    ...
                    placeholder="Events page title for Google"
              ...
              <div className="mx-auto w-full max-w-[820px] space-y-5 px-6 py-7">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setDraft((current) => ({
                        ...
                        slug:
                          !current.slug ||
                          current.slug === createEventSlug(current.title)
                            ? createEventSlug(event.target.value)
                            : current.slug,
                      }))
                    }
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setDraft((current) => ({
                        ...current,
                        slug: createEventSlug(event.target.value),
                      }))
                    }
                    className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                  ...
                  <p className="mt-1.5 text-xs text-slate-500">
                    Detail page: /event/{draft.slug || "your-slug"}
                  </p>
                </label>
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <select
                      value={
                        ...
                        Boolean(draft.category)
                          ? draft.category || DEFAULT_EVENT_CATEGORY
                          : categories[0] || DEFAULT_EVENT_CATEGORY
                      }
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                  ...
                  <label className="block">
                    ...
                    <select
                      value={normalizeEventType(draft.eventType)}
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          eventType: normalizeEventType(event.target.value),
                        }))
                      }
                      className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                    >
                  ...
                  >
                    {(draft.content || "").trim() ? (
                      ...
                      <p className="text-sm text-slate-400">
                        Write full event content… Click to open the rich text
                        editor.
                      </p>
                    )}
                    {(draft.content || "").trim() ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Optional meta for this event item.
                      </p>
                    </div>
                    <AiFieldButton
      ...
      {showContentEditor
        ? createPortal(
            <CustomSectionRichTextEditor
              ...
              placeholder="Write the complete event content..."
        ...
        <div className="fixed inset-0 z-[10100] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-[2px]">
          ...
          >
            ...
            >
              ...
              {eventToDelete.title.trim() || "this event"}”?
            </h3>
            <p className="mt-2 text-sm text-slate-600">
              This removes it from the Events page. You can’t undo this.
            </p>
      ...
      {bulkDeleteConfirm && selected.length > 0 ? (
        <div className="fixed inset-0 z-[10100] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-[2px]">
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="bulk-delete-event-title"
            className="w-[90%] max-w-sm rounded-xl bg-white p-6 text-center shadow-2xl"
          >
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-red-600">
              Delete events
            ...
            <h3
              id="bulk-delete-event-title"
              className="mt-2 text-lg font-semibold text-slate-950"
            >
              Delete {selected.length} selected event
              {selected.length === 1 ? "" : "s"}?
            </h3>
            <p className="mt-2 text-sm text-slate-600">
              This removes them from the Events page. You can&apos;t undo this.
            </p>
            <div className="mt-6 flex justify-center gap-3">
              <button
                type="button"
                onClick={() => {
                  deleteEventsByIds(selected);
